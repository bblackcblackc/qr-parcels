...
if (isset($oPOSTData->auth->login))
	{
		if ($oParcel->parcelSenderID != $cUser->userID)
			DropWithForbidden();
		
		$iOperationOwner = 0;//$cUser->userID;
		$i1stParam = $oParcel->CurrentCourier($mysqli);
		$i2ndParam = 0;//$cUser->userID;
		
		$iOperationCode = OPERATION_PARCEL_TO_COURIER;
	}
else
	{
		...
		$i1stParam = $cCourier->courierID;
		$i2ndParam = 0;//$oParcel->parcelSenderID;
		
		$iOperationCode = OPERATION_PARCEL_FROM_USER;
	}
